...
def load_model(cache_model):
    ...
    print("BERT模型设备：", next(bert_model.parameters()).device)
    bert_model.eval()
...
def Intent_Recognition(query, choice, progress_queue=None):
    """
    意图识别函数，支持进度报告和本地备用方案
    """
    # 检查Ollama是否可用
    if not check_ollama_connection():
        if progress_queue:
            progress_queue.put("Ollama未连接，使用本地意图识别...")
        print("Ollama未连接，使用本地意图识别...")
        result = local_intent_recognition(query)
        if progress_queue:
            progress_queue.put("本地意图识别完成")
        return result
...
"""
    ...
    try:
        # 发送进度信息
        if progress_queue:
            progress_queue.put("正在调用LLM进行意图识别...")
        ...
        if progress_queue:
            progress_queue.put("意图识别完成")
        
        return rec_result
    except Exception as e:
        print(f'意图识别出错: {str(e)}')
        if progress_queue:
            progress_queue.put(f"意图识别出错: {str(e)}")
...
def add_shuxing_prompt(entity, shuxing, client):
    ...
    try:
        ...
        add_prompt += "<提示>"
        add_prompt += "用户对{}可能有查询{}需求，知识库内容如下：".format(entity, shuxing)
        if len(res) > 0:
            join_res = "".join(res)
            add_prompt += join_res
        else:
...
def add_lianxi_prompt(entity, lianxi, target, client):
    ...
    try:
        ...
        add_prompt += "<提示>"
        add_prompt += "用户对{}可能有查询{}需求，知识库内容如下：".format(entity, lianxi)
        if len(res) > 0:
            join_res = "、".join(res)
            add_prompt += join_res
        else:
...
def generate_prompt(response, query, client, bert_model, bert_tokenizer, rule, tfidf_r, device, idx2tag):
    ...
    if '疾病症状' in entities and '疾病' not in entities:
        ...
        if len(res) > 0:
            ...
            all_en = "、".join(res)
            prompt += "<提示>用户有{}的情况，知识库推测其可能是得了{}。请注意这只是一个推测，你需要明确告知用户这一点。</提示>".format(entities['疾病症状'], all_en)
    pre_len = len(prompt)
    if "简介" in response:
        if '疾病' in entities:
    ...
    if "生产商" in response:
        try:
            ...
            res = client.run(sql_q).data()[0].values()
            prompt += "<提示>"
            prompt += "用户对{}可能有查询药品生产商的需求，知识图谱内容如下：".format(entities['药品'])
            if len(res) > 0:
                prompt += "".join(res)
            else:
                prompt += "图谱中无信息，查找失败"
...
def main(is_admin, usname):
    ...
    with st.sidebar:
        ...
        selected_option = st.selectbox(
            label='请选择大语言模型:',
            options=['qwen:4b', 'qwen3:4b', 'qwen3:4b']
        )
    ...
    if query := st.chat_input("Ask me anything!", key=f"chat_input_{active_window_index}"):
        ...
        if not ollama_available:
            status_placeholder.warning("⚠️ Ollama未连接，使用本地意图识别...")
        else:
            status_placeholder.info("✅ Ollama连接正常")
        
        # 开始意图识别
        status_placeholder.text("正在进行意图识别...")
        ...
        while intent_thread.is_alive():
            try:
                ...
                if not progress_queue.empty():
                    ...
                    status_placeholder.text(progress_msg)
                
                # 检查是否超时
                if time.time() - thread_start_time > 30:  # 30秒超时
                    status_placeholder.error("⚠️ 意图识别超时，尝试重新处理...")
                    break
        ...
        if not result_queue.empty():
            intent_response = result_queue.get()
        else:
            intent_response = "查询疾病简介"  # 默认返回
            status_placeholder.warning("⚠️ 意图识别超时，使用默认意图")
        
        status_placeholder.text("正在生成回答...")
        ...
        try:
            ...
            if check_ollama_connection():
                # 使用ollama流式输出
                for chunk in ollama.chat(model=choice, messages=[{'role': 'user', 'content': prompt}], stream=True, options={"temperature": 0.3, "num_gpu": -1}):
                    last += chunk['message']['content']
                    response_placeholder.markdown(last)
            else:
                # Ollama不可用时，直接使用知识库内容生成简单回答
                response_placeholder.text("正在使用本地知识库生成回答...")
                knowledge = re.findall(r'<提示>(.*?)</提示>', prompt)
                if knowledge:
                    last = "根据知识库信息：\n\n"
                    for idx, kn in enumerate(knowledge):
                        if len(kn) >= 3:
                            last += f"提示{idx + 1}: {kn}\n\n"
                else:
                    last = "根据已知信息无法回答该问题"
